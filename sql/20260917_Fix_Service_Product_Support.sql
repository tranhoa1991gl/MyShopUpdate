/*
    HoaTran POS - Bổ sung tính năng Sản phẩm Dịch vụ (không quản lý tồn kho).
    
    1. Đảm bảo cột is_service trên bảng dbo.products.
    2. Cập nhật stored procedure Report_GetInventoryMovement:
       - Loại trừ sản phẩm Dịch vụ khỏi SALES_OUT và RETURN_IN
       - Loại trừ sản phẩm Dịch vụ khỏi danh sách hiển thị thẻ kho / xuất nhập tồn vật lý.
...
-- 1. Đảm bảo cột is_service trên bảng products
IF NOT EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID(N'dbo.products') AND name = N'is_service'
)
BEGIN
    ALTER TABLE dbo.products ADD is_service BIT NOT NULL CONSTRAINT DF_products_is_service DEFAULT (0);
END;
GO

-- 2. Khởi tạo stored procedure trước nếu chưa có để lệnh ALTER PROCEDURE luôn thành công
...
BEGIN
    ...
    (
        ...
        -- Service products are excluded because they do not affect physical stock.
        SELECT
            oi.product_id,
            o.order_date AS movement_date,
...
PRINT N'Đã cập nhật cấu hình hỗ trợ sản phẩm Dịch vụ và báo cáo xuất nhập tồn.';
GO
