/*
    MyShop Online SQL Migration
    File: 2026-06-09-product-barcode-online-full.sql

    Mục đích:
    - Tổng hợp toàn bộ SQL từ lúc thêm Mã vạch sản phẩm.
    - Dùng để upload GitHub và chạy vá online cho khách.

    Nội dung:
    1) Thêm cột dbo.products.barcode nếu chưa có.
    2) Chuẩn hóa barcode: bỏ khoảng trắng đầu/cuối, chuỗi rỗng => NULL.
    3) Cập nhật các stored procedure sản phẩm để SELECT/INSERT/UPDATE/SEARCH có Barcode.
    4) Tạo unique index UX_products_barcode nếu dữ liệu hiện tại không có barcode trùng.

    Lưu ý an toàn:
    - Script idempotent: chạy nhiều lần không sao.
    - Nếu khách đang có barcode trùng, script KHÔNG báo lỗi dừng app; chỉ PRINT/SELECT danh sách trùng và bỏ qua tạo unique index.
      App vẫn chạy được, nhưng cần xử lý trùng rồi chạy lại script để SQL chặn trùng tuyệt đối.
*/
...
/* =========================================================
   ...
   ========================================================= */
CREATE OR ALTER PROCEDURE [dbo].[Products_All]
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [dbo].[Products_GetAvailable]
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [dbo].[Products_Search]
...
CREATE OR ALTER PROCEDURE [dbo].[Products_SearchByKeywordAndCategory]
...
CREATE OR ALTER PROCEDURE [dbo].[Products_SearchForImport]
    @Keyword NVARCHAR(200),
    @CategoryId INT
AS
BEGIN
...
CREATE OR ALTER PROCEDURE [dbo].[Products_GetSalesStatistics]
AS
BEGIN
    SET NOCOUNT ON;
