/*
  MigrationId: 20260908_sales_audit_followup
  Run on the shop database after backup and 20260908_sales_audit_reports.sql.
  Requires SQL_DB.sql (08/09/2026); do not change that original snapshot.
  Adds draft batch choices (NOT stock reservations) and corrects overview debt.
  Idempotent: no existing orders, stock, receipts or customer rows are rewritten.
  Safe standalone and inside the updater's transaction.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF DB_NAME() IN (N'master',N'model',N'msdb',N'tempdb')
   ...
   OR COL_LENGTH(N'dbo.customer_debt_adjustments',N'adjustment_type') IS NULL
   OR OBJECT_ID(N'dbo.Report_GetOverview',N'P') IS NULL
    THROW 51411,N'Chọn đúng database cửa hàng có schema SQL_DB.sql trước khi chạy migration.',1;

DECLARE @OwnTransaction BIT = CASE WHEN @@TRANCOUNT=0 THEN 1 ELSE 0 END;
BEGIN TRY
    IF @OwnTransaction=1 BEGIN TRANSACTION;
    ELSE SAVE TRANSACTION SalesAuditFollowup;

    IF OBJECT_ID(N'dbo.order_item_batch_selections',N'U') IS NULL
    BEGIN
        CREATE TABLE dbo.order_item_batch_selections
        (
            ...
            CONSTRAINT PK_order_item_batch_selections PRIMARY KEY(order_item_id,batch_id),
            CONSTRAINT FK_order_item_batch_selections_item FOREIGN KEY(order_item_id)
                REFERENCES dbo.order_items(order_item_id) ON DELETE CASCADE,
            CONSTRAINT CK_order_item_batch_selections_quantity CHECK(quantity_base>0),
            CONSTRAINT CK_order_item_batch_selections_sequence CHECK(selection_order>=0)
        );
        -- Deliberately no FK to product_batches: a draft must not prevent
        -- cancelling/deleting an import. Confirmation revalidates every choice.
    END;
...
BEGIN CATCH
    ...
    ELSE IF @OwnTransaction=0 AND XACT_STATE()=1 ROLLBACK TRANSACTION SalesAuditFollowup;
    THROW;
END CATCH;
